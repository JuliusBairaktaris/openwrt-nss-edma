 ...
 *
 * The phase-2a identity redirect (debugfs identity_mask) is kept as a
 * plumbing regression tool; identity and fw arming are mutually
 * exclusive per port.
 *
...
struct ppe_nss_port {
	int if_num;
	struct net_device *netdev;	/* DSA user port, held while armed/identity */
	struct net_device *conduit;	/* qca-edma conduit, held while armed/identity */
	bool armed;			/* exposed to nss-drv at next registration */
	bool identity;			/* identity TX redirect installed (test mode) */
...
static unsigned long ppe_nss_identity_mask;
...
/*
 * ===== identity TX redirect (phase 2a regression tool) =====
 */

/*
 * Identity TX redirect: count the frame and hand it straight back to
 * the host EDMA TX path. Runs in the conduit's ndo_start_xmit context.
 */
static netdev_tx_t ppe_nss_identity_xmit(struct sk_buff *skb, void *ctx)
{
	struct ppe_nss_port *port = ctx;

	atomic64_inc(&port->tx_redirect_pkts);
	return qca_edma_conduit_xmit(port->conduit, skb);
}

/*
 * Attach/detach run with ppe_nss_lock held; attach additionally needs
 * rtnl held by the caller (netdev iteration). Lock order is always
 * rtnl -> ppe_nss_lock, matching the NETDEV_UNREGISTER notifier path.
 */
static int ppe_nss_identity_attach(int if_num)
{
	struct ppe_nss_port *port = &ppe_nss_ports[if_num];
	int ret;
	...
	ret = qca_edma_port_tx_redirect_set(port->conduit, if_num,
					    ppe_nss_identity_xmit, port);
	if (ret) {
		ppe_nss_port_put(port);
		return ret;
	}
	...
	port->identity = true;
	__set_bit(if_num, &ppe_nss_identity_mask);
	netdev_info(port->netdev, "qca-ppe-nss: identity TX redirect attached (port %d)\n",
		    if_num);
	return 0;
}

static void ppe_nss_identity_detach(int if_num)
{
	struct ppe_nss_port *port = &ppe_nss_ports[if_num];
	...
	/* Returns with no handler invocation in flight */
	qca_edma_port_tx_redirect_clear(port->conduit, if_num);

	netdev_info(port->netdev,
		    "qca-ppe-nss: identity TX redirect detached (port %d, %lld pkts)\n",
		    if_num, (long long)atomic64_read(&port->tx_redirect_pkts));

	ppe_nss_port_put(port);
	port->identity = false;
	__clear_bit(if_num, &ppe_nss_identity_mask);
}

static int ppe_nss_identity_mask_apply(unsigned long mask)
{
	int ret = 0;
	int i;
	...
	for (i = NSS_DP_START_IFNUM; i < NSS_DP_MAX_INTERFACES; i++) {
		if (test_bit(i, &mask) &&
		    !test_bit(i, &ppe_nss_identity_mask)) {
			ret = ppe_nss_identity_attach(i);
			if (ret)
				break;
		} else if (!test_bit(i, &mask) &&
			   test_bit(i, &ppe_nss_identity_mask)) {
			ppe_nss_identity_detach(i);
		}
	}
	mutex_unlock(&ppe_nss_lock);
	rtnl_unlock();
...
static ssize_t ppe_nss_identity_mask_write(struct file *file,
					   const char __user *ubuf,
					   size_t len, loff_t *ppos)
{
	unsigned long mask;
	...
	ret = ppe_nss_identity_mask_apply(mask);
	if (ret)
		return ret;
...
static int ppe_nss_identity_mask_show(struct seq_file *m, void *v)
{
	seq_printf(m, "0x%lx\n", ppe_nss_identity_mask);
	return 0;
}

static int ppe_nss_identity_mask_open(struct inode *inode, struct file *file)
{
	return single_open(file, ppe_nss_identity_mask_show, NULL);
}

static const struct file_operations ppe_nss_identity_mask_fops = {
	.owner = THIS_MODULE,
	.open = ppe_nss_identity_mask_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
	.write = ppe_nss_identity_mask_write,
};
...
static int ppe_nss_status_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "identity_mask: 0x%lx\n", ppe_nss_identity_mask);
	seq_printf(m, "fw_mask: 0x%lx\n", ppe_nss_fw_mask);
	for (i = NSS_DP_START_IFNUM; i < NSS_DP_MAX_INTERFACES; i++) {
		struct ppe_nss_port *port = &ppe_nss_ports[i];

		seq_printf(m,
			   "if_num %d: netdev=%s armed=%d identity=%d overridden=%d started=%d fw_vsi=%d tx_redirect_pkts=%lld tx_busy=%lld rx_fw_pkts=%lld\n",
			   i,
			   port->netdev ? netdev_name(port->netdev) : "-",
			   port->armed, port->identity, port->overridden,
...
static int __init qca_ppe_nss_init(void)
{
	...
	debugfs_create_file("status", 0444, ppe_nss_dentry, NULL,
			    &ppe_nss_status_fops);
	debugfs_create_file("identity_mask", 0644, ppe_nss_dentry, NULL,
			    &ppe_nss_identity_mask_fops);
...
static void __exit qca_ppe_nss_exit(void)
{
	 ...
	 * no port can be overridden or started here; only identity
	 * redirects and armed-but-idle ports remain.
	 */
	mutex_lock(&ppe_nss_lock);
	for (i = NSS_DP_START_IFNUM; i < NSS_DP_MAX_INTERFACES; i++) {
		struct ppe_nss_port *port = &ppe_nss_ports[i];
